...
def claim_color_for_id(peer_id, chosen):
    """Atomically checks that `chosen` is still free (ignoring peer_id's own
    current color) and assigns it if so. Returns True on success. Doing the
    check-and-set under one lock acquisition avoids a race where two
    concurrent callers could both see a color as free and both take it."""
    with color_lock:
        used = set(idx for pid, idx in id_colors.items() if pid != peer_id)
        if chosen in used:
            return False
...
def handle_incoming(raw, ip):
    ...
    if parts[0] == "MSG" and len(parts) == 5:
    ...
    elif parts[0] == "PRIV" and len(parts) == 5:
    ...
    elif parts[0] == "COLOR" and len(parts) == 3:
    ...
    elif parts[0] == "NAME" and len(parts) == 3:
    ...
    elif parts[0] == "PING" and len(parts) == 4:
    ...
    elif parts[0] == "PONG" and len(parts) == 4:
        _, target_id, peer_id, sent_time = parts
        if len(peer_id) != 8 or len(target_id) != 8:
            return
        if not verify_sender(is_direct, ip, peer_id, signed_line, tag):
    ...
    elif parts[0] == "FILE_OFFER" and len(parts) == 7:
    ...
    elif parts[0] == "FILE_DECLINE" and len(parts) == 4:
    ...
    elif parts[0] == "FILE_ACCEPT" and len(parts) == 4:
        _, target_id, peer_id, offer_id = parts
        if len(peer_id) != 8 or len(target_id) != 8:
            return
        if not verify_sender(is_direct, ip, peer_id, signed_line, tag):
    ...
    elif parts[0] == "FILE" and len(parts) == 6:
    ...
    elif parts[0] == "FILE_ACK" and len(parts) == 5:
...
def resolve_peer_id(target):
    ...
    *different* "Alice" (e.g. an impersonator who deliberately reused a
    ...
    with names_lock:
        if len(target) == 8 and target in peer_names:
            return target
...
def send_private_message(target, text):
    ...
    line = f"{timestamp()} [private to {target}] {COLORS[my_color]}{MY_NAME}{RESET}: {text}"
    log_message(f"{timestamp()} [private to {target}] {MY_NAME}: {text}")
    ui_print(line)
...
def print_help():
    ...
    ui_print("  /color random             pick a random free color")
    ...
    ui_print("  /msg <name|id> <text>     send a private message to one peer")
...
def print_colors():
    ...
    ui_print("Use /color <n> to change, or /color random for a random free one.")
...
def process_command(message):
    ...
    global MY_NAME, SHOW_ID_INSTEAD_OF_NAME
    ...
    if message == "/quit":
        ...
        arg = message[len("/color"):].strip()
        with color_lock:
            used = set(idx for pid, idx in id_colors.items() if pid != MY_ID)
            current = id_colors.get(MY_ID)

        if arg == "random":
            free = [i for i in range(len(COLORS)) if i not in used and i != current]
            if not free:
                ui_print("No free colors left.")
            else:
                remaining = list(free)
                claimed = None
                while remaining and claimed is None:
                    pick_idx = uuid.uuid4().int % len(remaining)
                    candidate = remaining.pop(pick_idx)
                    if claim_color_for_id(MY_ID, candidate):
                        claimed = candidate
                if claimed is None:
                    ui_print("No free colors left.")
                else:
                    remember_color_for_name(MY_NAME, claimed)
                    send_to_all(wrap_relay(f"COLOR|{MY_ID}|{claimed}"))
                    ui_print(f"Color set to {COLORS[claimed]}{claimed}{RESET}")
        elif arg.isdigit():
            chosen = int(arg)
            if chosen < 0 or chosen >= len(COLORS):
                ui_print(f"Pick a number from 0 to {len(COLORS)-1}.")
            elif not claim_color_for_id(MY_ID, chosen):
                with color_lock:
                    still_used = set(idx for pid, idx in id_colors.items() if pid != MY_ID)
                free_now = [i for i in range(len(COLORS)) if i not in still_used]
                if free_now:
                    free_str = ", ".join(str(i) for i in free_now)
                    ui_print(f"That color is already taken. Free colors: {free_str} (or /color random).")
                else:
                    ui_print("That color is already taken, and no colors are currently free.")
            else:
        ...
        MY_NAME = message[6:].strip() or MY_NAME
        save_name(MY_NAME)
        ...
        rest = message[len("/sendfile "):].strip()
        args = rest.split(" ", 1)
        if len(args) == 2:
        ...
        target = message[len("/ignore "):].strip()
        if target:
            ignore_peer(target)
        else:
        ...
        target = message[len("/unignore "):].strip()
        if target:
        ...
        target_ip = message[len("/connect "):].strip()
        if target_ip:
...
def run_gui():
    ...
    # GUI themes are intentionally local to run_gui().  Theme switching is a
    # presentation concern, so the shared networking/command core does not need
    # any global theme state or references to Tk widgets.
    THEMES = {
        "classic": {
            "bg": "#0a0e0a",
            "panel": "#0f140f",
    ...
    peer_count_var = tk.StringVar(value="0")
    peer_label = tk.Label(
        header, textvariable=peer_count_var, fg=FG, bg=BG, font=SMALL_BOLD
    )
    peer_label.pack(side="right")
    peers_text_label = tk.Label(header, text="peers ", fg=DIM_FG, bg=BG, font=SMALL_FONT)
    peers_text_label.pack(side="right")
    ...
    COMMANDS = [
        ...
        ("/colors", "list available color numbers"),
        ("/color <n>", "change your color"),
    ...
    def apply_theme(name):
        ...
        peer_label.configure(bg=bg, fg=fg)
        peers_text_label.configure(bg=bg, fg=dim)
