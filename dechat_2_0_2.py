...
def _handle_priv(parts, is_direct, ip, signed_line, tag):
    ...
    if UI_MODE == "gui":
        ...
        ui_print("@@DECHAT_DM@@" + json.dumps({
            "peer_id": sender_id, "name": name, "text": text,
...
def send_private_message(target, text):
    ...
    if UI_MODE == "gui":
        ui_print("@@DECHAT_DM@@" + json.dumps({
            "peer_id": peer_id, "name": resolved_name, "text": text,
...
def run_gui():
    ...
    dm_conversations = {}   # peer_id -> {name, messages[(outgoing,name,text,color_idx)]}
    dm_order = []
    online_dm_order = []    # [(peer_id, display_name)] currently reachable; refreshed periodically
    selected_dm = {"peer_id": None}
    ...
    def _refresh_online_dm_list():
        rows = [(pid, name) for pid, name, _, _ in _reachable_people() if pid != MY_ID]
    ...
    def _refresh_dm_list():
        ...
        for pid in dm_order:
            dm_list.insert("end", dm_conversations[pid]["name"])
        if selected_dm["peer_id"] in dm_order:
            idx = dm_order.index(selected_dm["peer_id"])
            dm_list.selection_set(idx)
            dm_list.see(idx)

    def open_dm(peer_id, name=None):
        if peer_id == MY_ID:
            return
        if peer_id not in dm_conversations:
            with names_lock:
                resolved = name or peer_names.get(peer_id, peer_id)
            dm_conversations[peer_id] = {"name": resolved, "messages": []}
            dm_order.append(peer_id)
        elif name:
            dm_conversations[peer_id]["name"] = name
        selected_dm["peer_id"] = peer_id
        _refresh_dm_list()
        _render_transcript(dm_log, dm_conversations[peer_id]["messages"])
    ...
    def _on_dm_select(event=None):
        ...
        if not sel:
            return
        pid = dm_order[sel[0]]
        selected_dm["peer_id"] = pid
        _render_transcript(dm_log, dm_conversations[pid]["messages"])
        if active_tab.get() == "dms":
            prompt_var.set(f"DM @{dm_conversations[pid]['name']}>")

    def _on_online_dm_select(event=None):
        sel = online_dm_list.curselection()
        if not sel or sel[0] >= len(online_dm_order):
            return
        pid, name = online_dm_order[sel[0]]
    ...
    def handle_dm_event(data):
        ...
        if pid not in dm_conversations:
            dm_conversations[pid] = {"name": name, "messages": []}
            dm_order.append(pid)
        else:
            dm_conversations[pid]["name"] = name
        dm_conversations[pid]["messages"].append((
            bool(data.get("outgoing")), MY_NAME if data.get("outgoing") else name,
            str(data.get("text", "")), int(data.get("color_idx", 0)),
        ))
        _refresh_dm_list()
        if selected_dm["peer_id"] == pid:
            _render_transcript(dm_log, dm_conversations[pid]["messages"])
    ...
    def switch_conversation_tab(tab):
        ...
        if tab == "chat":
            ...
            pid = selected_dm["peer_id"]
            prompt_var.set(f"DM @{dm_conversations[pid]['name']}>" if pid in dm_conversations else "DM> ")
        else:
    ...
    def show_people_window():
        ...
        for pid, name, color_idx, ip_text in rows:
            ...
            if pid != MY_ID:
                tk.Button(row, text="DM", command=lambda p=pid, n=name, w=win: (w.destroy(), open_dm(p, n)), bg=theme["panel"], fg=theme["accent"], activebackground=theme["dim"], activeforeground=theme["bg"], relief="flat", font=SMALL_FONT).pack(side="right")
