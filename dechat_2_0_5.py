...
def show_gui_update_lock(lock_root, BG, PANEL_BG, FG, DIM_FG, ACCENT, FONT_FAMILY):
    ...
    Reuses the single process-wide root window (passed in as `lock_root`)
    instead of creating a second tk.Tk() -- a second Tcl interpreter can
    leave the process in a broken state once destroyed, which is exactly
    what a prior version of this function did."""
...
# Markdown + LaTeX rendering for the GUI chat/DM/group logs
#
# GitHub-flavored markdown (headers, bold/italic/strikethrough, inline and
# fenced code, blockquotes, ordered/unordered/task lists, tables, hr,
# links) plus a lightweight LaTeX-to-Unicode converter for $inline$ and
# $$display$$ math. There is no real typesetting engine available without
# third-party packages, so math is approximated with Unicode Greek
# letters, super/subscripts, and common operators rather than actually
# laid out -- good enough to read formulas inline in a chat log. Every
# styled run is inserted into the target tk.Text widget via its own
# `insert(index, text, tags)` call, matching how the rest of the GUI
# already tags colored/system/error text, so markdown tags compose with
# the existing per-sender/ANSI color tags instead of replacing them.
MD_LATEX_GREEK = {
...
def latex_to_unicode(expr):
    """Best-effort, stdlib-only conversion of a LaTeX math snippet into a
    readable Unicode approximation (Greek letters, super/subscripts,
    fractions, common operators)."""
    expr = expr.strip()

    def repl_frac(m):
        return f"({m.group(1)}/{m.group(2)})"
    for _ in range(4):
        new = re.sub(r"\\frac\{([^{}]*)\}\{([^{}]*)\}", repl_frac, expr)
        if new == expr:
            break
        expr = new
    ...
    def repl_sqrt(m):
        return "\u221a(" + m.group(1) + ")"
    for _ in range(4):
        new = re.sub(r"\\sqrt\{([^{}]*)\}", repl_sqrt, expr)
        if new == expr:
            break
        expr = new
    ...
    def repl_cmd(m):
        name = m.group(1)
        return MD_LATEX_GREEK.get(name, "\\" + name)
    expr = re.sub(r"\\([A-Za-z]+)", repl_cmd, expr)

    def repl_sup(m):
        body = m.group(1) if m.group(1) is not None else m.group(2)
        mapped, ok = _md_map_chars(body, MD_SUPERSCRIPTS)
        return mapped if ok else "^" + body
    expr = re.sub(r"\^\{([^{}]*)\}|\^(\w)", repl_sup, expr)

    def repl_sub(m):
        body = m.group(1) if m.group(1) is not None else m.group(2)
        mapped, ok = _md_map_chars(body, MD_SUBSCRIPTS)
        return mapped if ok else "_" + body
    expr = re.sub(r"_\{([^{}]*)\}|_(\w)", repl_sub, expr)
...
_MD_INLINE_SPLIT_RE = re.compile(
    r"(\*\*\*.+?\*\*\*|\*\*.+?\*\*|__.+?__|\*[^*\n]+?\*|_[^_\n]+?_|"
    r"~~.+?~~|`[^`\n]+?`|\$\$.+?\$\$|\$[^$\n]+?\$|"
    r"!\[[^\]]*?\]\([^)\s]+?\)|\[[^\]]+?\]\([^)\s]+?\))"
)
...
def _md_tokenize_inline(text):
    return [p for p in _MD_INLINE_SPLIT_RE.split(text) if p != ""]


def _md_classify_inline(token):
    if token.startswith("$$") and token.endswith("$$") and len(token) >= 4:
        return ("math", token[2:-2])
    if token.startswith("$") and token.endswith("$") and len(token) >= 2:
        return ("math", token[1:-1])
    if token.startswith("***") and token.endswith("***") and len(token) >= 6:
        return ("bolditalic", token[3:-3])
    if token.startswith("**") and token.endswith("**") and len(token) >= 4:
        return ("bold", token[2:-2])
    if token.startswith("__") and token.endswith("__") and len(token) >= 4:
        return ("bold", token[2:-2])
    if token.startswith("~~") and token.endswith("~~") and len(token) >= 4:
        return ("strike", token[2:-2])
    if token.startswith("`") and token.endswith("`") and len(token) >= 2:
        return ("code", token[1:-1])
    if token.startswith("!["):
...
_MD_TASK_RE = re.compile(r"^\[( |x|X)\]\s+(.*)$")
...
MD_TAG_NAMES = {
    ...
    "italic": "md_italic",
    "strike": "md_strike",
    "inline_code": "md_inline_code",
    "code_block": "md_code_block",
    "math": "md_math",
    "link": "md_link",
    "hr": "md_hr",
    "blockquote": "md_blockquote",
    "blockquote_bar": "md_blockquote_bar",
    "list_marker": "md_list_marker",
    "task_box": "md_task_box",
    "task_done": "md_task_done",
    "table_header": "md_table_header",
    "table_rule": "md_table_rule",
    "table_row": "md_table_row",
    "header": ["md_h1", "md_h2", "md_h3", "md_h4", "md_h5", "md_h6"],
}
...
def render_inline(widget, text, base_tags, extra_tag, tags):
    """Renders one line's worth of inline markdown (bold/italic/strike,
    inline code, $math$, links) into `widget` ending at "end"."""
    base = tuple(base_tags) if base_tags else ()
    extra = (extra_tag,) if extra_tag else ()

    def insert(run_text, *style_tags):
        if run_text == "":
            return
        widget.insert("end", run_text, base + extra + tuple(t for t in style_tags if t))

    for token in _md_tokenize_inline(text):
        kind, *rest = _md_classify_inline(token)
        if kind == "text":
            insert(rest[0])
        elif kind == "bold":
            insert(rest[0], tags["bold"])
        elif kind == "italic":
            insert(rest[0], tags["italic"])
        elif kind == "bolditalic":
            insert(rest[0], tags["bold"], tags["italic"])
        elif kind == "strike":
            insert(rest[0], tags["strike"])
        elif kind == "code":
            insert(rest[0], tags["inline_code"])
        elif kind == "math":
            insert(latex_to_unicode(rest[0]), tags["math"])
        elif kind == "link":
            label, _url = rest
            insert(label, tags["link"])
        elif kind == "image":
            label, url = rest
            insert(f"[image: {label or url}]", tags["link"])


def render_markdown_into(widget, text, base_tags=(), md_tags=None):
    """Parses `text` as GitHub-flavored markdown (with $..$/$$..$$ LaTeX
    math) and inserts the styled result into a tk.Text/ScrolledText
    `widget` ending at "end". `base_tags` are extra tags applied to every
    inserted run (e.g. a peer-color tag) so markdown styling composes
    with the existing per-sender coloring instead of overriding it."""
    tags = md_tags or MD_TAG_NAMES
    base = tuple(base_tags) if base_tags else ()

    def insert(run_text, *extra):
        if run_text == "":
            return
        widget.insert("end", run_text, base + tuple(extra))
    ...
    while i < n:
        ...
        fence_m = _MD_FENCE_RE.match(line)
        if fence_m:
        ...
        header_m = _MD_HEADER_RE.match(line)
        if header_m:
            level = len(header_m.group(1))
            content = header_m.group(2)
            render_inline(widget, content, base, tags["header"][min(level - 1, 5)], tags)
            insert("\n")
            i += 1
            continue

        bq_m = _MD_BLOCKQUOTE_RE.match(line)
        if bq_m and line.strip().startswith(">"):
            quote_lines = []
            while i < n and _MD_BLOCKQUOTE_RE.match(lines[i]) and lines[i].strip().startswith(">"):
                quote_lines.append(_MD_BLOCKQUOTE_RE.match(lines[i]).group(1))
                i += 1
            insert("\u2503 ", tags["blockquote_bar"])
            render_inline(widget, " ".join(quote_lines), base, tags["blockquote"], tags)
            insert("\n")
            continue

        if (i + 1 < n) and _MD_TABLE_SEP_RE.match(lines[i + 1]) and "|" in line:
            header_cells = _md_split_table_row(line)
            i += 2
            rows = []
            ...
            insert(fmt_row(header_cells) + "\n", tags["table_header"])
            insert("\u2500" * (sum(widths) + 3 * (len(widths) - 1)) + "\n", tags["table_rule"])
            for row in rows:
                insert(fmt_row(row) + "\n", tags["table_row"])
            continue

        ul_m = _MD_ULIST_RE.match(line)
        ol_m = _MD_OLIST_RE.match(line)
        if ul_m or ol_m:
            if ul_m:
                indent, _marker, content = ul_m.groups()
            else:
                indent, num, content = ol_m.groups()
            depth = len(indent) // 2
            task_m = _MD_TASK_RE.match(content)
            pad = "  " * depth
            if task_m:
                checked = task_m.group(1).lower() == "x"
                box = "\u2611" if checked else "\u2610"
                insert(f"{pad}{box} ", tags["task_box"])
                render_inline(widget, task_m.group(2), base,
                              tags["task_done"] if checked else None, tags)
            else:
                bullet = "\u2022" if ul_m else f"{num}."
                insert(f"{pad}{bullet} ", tags["list_marker"])
                render_inline(widget, content, base, None, tags)
            insert("\n")
            i += 1
            continue

        render_inline(widget, line, base, None, tags)
        insert("\n")
        i += 1

    if in_code_fence and fence_buffer:
        flush_code_block()


def configure_markdown_tags(widget, font_family, base_size, colors):
    """Registers every markdown tag used by render_markdown_into() on the
    given Text widget, styled to resemble GitHub's markdown rendering
    (bold headers of decreasing size, shaded code, a left-barred
    blockquote, aligned tables) using the given theme's colors."""
    fg = colors["fg"]
    dim = colors["dim"]
    accent = colors["accent"]
    panel = colors["panel"]

    widget.tag_configure(MD_TAG_NAMES["bold"], font=(font_family, base_size, "bold"))
    widget.tag_configure(MD_TAG_NAMES["italic"], font=(font_family, base_size, "italic"))
    widget.tag_configure(MD_TAG_NAMES["strike"], font=(font_family, base_size, "overstrike"))
    widget.tag_configure(MD_TAG_NAMES["inline_code"], font=(font_family, base_size),
                          background=panel, foreground=accent)
    widget.tag_configure(MD_TAG_NAMES["code_block"], font=(font_family, base_size),
                          background=panel, foreground=fg, lmargin1=14, lmargin2=14)
    widget.tag_configure(MD_TAG_NAMES["math"], font=(font_family, base_size, "italic"),
                          foreground=accent)
    widget.tag_configure(MD_TAG_NAMES["link"], font=(font_family, base_size, "underline"),
                          foreground=accent)
    widget.tag_configure(MD_TAG_NAMES["hr"], foreground=dim)
    widget.tag_configure(MD_TAG_NAMES["blockquote"], font=(font_family, base_size, "italic"),
                          foreground=dim, lmargin1=14, lmargin2=14)
    widget.tag_configure(MD_TAG_NAMES["blockquote_bar"], foreground=dim)
    widget.tag_configure(MD_TAG_NAMES["list_marker"], foreground=accent)
    widget.tag_configure(MD_TAG_NAMES["task_box"], foreground=accent)
    widget.tag_configure(MD_TAG_NAMES["task_done"], font=(font_family, base_size, "overstrike"),
                          foreground=dim)
    widget.tag_configure(MD_TAG_NAMES["table_header"], font=(font_family, base_size, "bold"))
    widget.tag_configure(MD_TAG_NAMES["table_rule"], foreground=dim)
    widget.tag_configure(MD_TAG_NAMES["table_row"], font=(font_family, base_size))
    sizes = [base_size + 8, base_size + 6, base_size + 4, base_size + 2, base_size + 1, base_size]
    for idx, tagname in enumerate(MD_TAG_NAMES["header"]):
        widget.tag_configure(tagname, font=(font_family, sizes[idx], "bold"), foreground=fg,
...
def run_gui():
    ...
    # There must only ever be one tk.Tk() root per process: Tk() spins up
    # its own Tcl interpreter, and creating a second one (even briefly, to
    # probe something, then destroying it) can leave the *real* window
    # broken afterwards -- blank content or spurious Tcl-level errors
    # ("application has been destroyed") on some platforms, notably
    # macOS's older Apple-bundled Tk. So the root window is created first,
    # unconditionally, and reused for both the font probe below and
    # (if needed) the update-lock screen, instead of each making its own.
    root = tk.Tk()
    root.withdraw()  # hidden until we know which screen (lock vs chat) to show

    # Pick a monospace font up front (before the chat UI is built) so both
    # the lock screen and the main chat UI use the same resolved family.
    _mono_candidates = [
    ...
    if protocol_version is not None and protocol_version != PROTOCOL_VERSION:
        show_gui_update_lock(root, BG, PANEL_BG, FG, DIM_FG, ACCENT, FONT_FAMILY)
        return
    ...
    configure_markdown_tags(chat_log, FONT_FAMILY, FONT[1], _initial_theme)
    ...
    for _text_widget in (dm_log, group_log):
        ...
        configure_markdown_tags(_text_widget, FONT_FAMILY, FONT[1], _initial_theme)
    ...
    def _render_transcript(widget, messages):
        ...
        for outgoing, sender_name, text, color_idx in messages:
            ...
            if outgoing:
                ...
                render_markdown_into(widget, text)
                widget.insert("end", "\n")
            else:
    ...
    _msg_prefix_re = _re.compile(r"^(?:\d\d:\d\d )?[^:\n]+: ")

    def append_log_markdown(text, extra_tag=None):
        """Like append_log_ansi, but for lines shaped like a chat message
        ("HH:MM name: body" or "name: body"): the "HH:MM name: " prefix
        keeps its normal ANSI-derived tag, and only the message body after
        it is parsed as markdown. Lines that don't match that shape (no
        recognizable "name: " prefix) fall back to plain ANSI rendering
        so system/status lines are never misparsed as markdown."""
        stripped_for_match = strip_ansi(text)
        prefix_m = _msg_prefix_re.match(stripped_for_match)
        if not prefix_m:
            append_log_ansi(text, extra_tag)
            return
        prefix_len = prefix_m.end()
        # Walk the original (possibly ANSI-colored) string far enough to
        # consume the same number of visible characters as the prefix
        # match, so escape codes inside the prefix are preserved and only
        # the trailing message body is treated as markdown.
        visible = 0
        cut = 0
        for part in _ansi_split_re.split(text):
            if part in ANSI_TO_TAG or part == "\033[0m":
                cut += len(part)
                continue
            remaining = prefix_len - visible
            if remaining <= 0:
                break
            if len(part) <= remaining:
                ...
                cut += remaining
                break
        prefix_part, body_part = text[:cut], text[cut:]
        trailing_newline = body_part.endswith("\n")
        if trailing_newline:
            body_part = body_part[:-1]
        append_log_ansi(prefix_part, extra_tag)
        chat_log.configure(state="normal")
        # extra_tag ("me"/"pinged") is meant for the sender-name prefix,
        # not the message body -- "me" in particular sets a bold font,
        # which would make every outgoing message body bold regardless of
        # markdown and hide **bold** entirely. Body markup should come
        # only from render_markdown_into's own tags.
        render_markdown_into(chat_log, body_part)
        chat_log.configure(state="disabled")
        chat_log.see("end")
    ...
    def classify_and_append(line):
        ...
        if raw.startswith("\a<<PING>>") and raw.endswith("<<PING>>"):
            ...
            append_log_markdown(maybe_hide_timestamp(raw) + "\n", "me")
    ...
    def apply_theme(name):
        ...
        configure_markdown_tags(chat_log, FONT_FAMILY, FONT[1], theme)
        ...
        for text_widget in (dm_log, group_log):
            ...
            configure_markdown_tags(text_widget, FONT_FAMILY, FONT[1], theme)
    ...
    def refresh_peer_count():
        # Keep the header count identical to /who, which lists direct TCP
        # connections from the shared `peers` dictionary. Mesh routes remain
        # available in the People window and online DM list, but they are not
        # counted here as connected peers.
        with lock:
            count = len(peers)
